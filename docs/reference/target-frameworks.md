...
| | | uap10.0 |
| | | uap10.0.xxxxx (где 10.0.xxxxx — Минимальная версия целевой платформы принимающего приложения) |
...
> [!Warning]
> **Использовать переносимые библиотеки классов не рекомендуется**. Несмотря на то, что переносимые библиотеки классов поддерживаются, вместо них авторам следует реализовать поддержку netstandard. Платформы .NET Standard является развитием переносимых библиотек классов и переносимость двоичных файлов между платформами с использованием единого моникера, который не привязано к статической библиотеки, такие как *переносимой-a + b + c* моникеры.
