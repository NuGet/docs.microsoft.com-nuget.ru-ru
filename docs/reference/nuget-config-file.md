...
| сигнатуревалидатионмоде | Указывает режим проверки, используемый для проверки сигнатур пакетов при установке пакета и восстановления. Значения: `accept`, `require`. По умолчанию равен `accept`.
...
| Ключ | Значение |
| --- | --- |
| format | Логическое значение, указывающее формат управления пакетами по умолчанию. Если `1`, то используется формат PackageReference. Если `0`, то format имеет формат *Packages. config*. |
