...
При использовании пользовательской лицензии, которая не поддерживается в выражениях лицензий, можно упаковать `.txt` или `.md` файл с текстом лицензии. Например:
...
#### <a name="copyright"></a>уведомления об авторских правах;
...
Метаданные репозитория, состоящие из четырех необязательных атрибутов: `type` и `url` *(4.0 +)*, а `branch` и `commit` *(4.6 +)*. Эти атрибуты позволяют сопоставлять `.nupkg` с репозиторием, в котором он построен, с возможностью получения таких сведений, как имя отдельной ветви и/или фиксация хэша SHA-1, который создал пакет. Это должен быть общедоступный URL-адрес, который может вызываться непосредственно программным обеспечением управления версиями. Это не должна быть HTML-страница, так как она предназначена для компьютера. Вместо этого для связывания с страницей проекта используйте поле `projectUrl`.
...
Узел `<package>` может содержать узел `<files>` в качестве одноуровневого элемента для `<metadata>`и `<contentFiles>` дочерний элемент в `<metadata>`, чтобы указать, какие файлы сборки и содержимого нужно включить в пакет. Дополнительные сведения см. далее в разделах [Включение файлов сборки](#including-assembly-files) и [Включение файлов содержимого](#including-content-files) этой статьи.
...
При создании пакета [команда `nuget pack`](../reference/cli-reference/cli-ref-pack.md) заменяет маркеры с разделителями $ в узле `.nuspec` файла `<metadata>` значениями из файла проекта или параметра `pack` команды `-properties`.
...
> [!Important]
> При создании `.nuspec` из проекта, использующего `nuget spec`, зависимости, которые существуют в этом проекте, не включаются автоматически в результирующий `.nuspec` файл. Вместо этого используйте `nuget pack myproject.csproj`и получите файл *. nuspec* из созданного *nupkgного* файла. Этот *nuspec* содержит зависимости.
...
> [!Note]
> Формат [моникера целевой платформы (TFM)](../reference/target-frameworks.md) , используемый в `lib/ref` папке, отличается по сравнению с TFM, используемым в `dependency groups`. Если целевые платформы, объявленные в `dependencies group` и в папке `lib/ref` файла `.nuspec`, не имеют точных совпадений, то `pack` команда вызовет [предупреждение NUGET NU5128](../reference/errors-and-warnings/nu5128.md).
...
Каждому элементу `<group>` требуется атрибут `targetFramework` и ноль или более элементов `<frameworkReference>`.
