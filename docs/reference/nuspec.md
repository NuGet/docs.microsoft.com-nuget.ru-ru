...
- Используйте `.nuspec` с `nuget.exe pack` для проектов в стиле, отличных от SDK, использующих `packages.config`.

- Файл `.nuspec` не требуется для создания пакетов для проектов в [стиле SDK](../resources/check-project-format.md) (обычно это проекты .NET Core и .NET Standard, использующие [атрибут SDK](/dotnet/core/tools/csproj#additions)). (Обратите внимание, что при создании пакета создается `.nuspec`.)

   При создании пакета с использованием `dotnet.exe pack` или `msbuild pack target` рекомендуется включить в файл проекта [все свойства](../reference/msbuild-targets.md#pack-target) , которые обычно находятся в файле `.nuspec`. Однако вместо этого можно [использовать файл `.nuspec` для упаковки с помощью `dotnet.exe` или `msbuild pack target`](../reference/msbuild-targets.md#packing-using-a-nuspec).

- Для проектов, перенесенных из `packages.config` в [PackageReference](../consume-packages/package-references-in-project-files.md), для создания пакета не требуется файл `.nuspec`. Вместо этого используйте [MSBuild-т:ПАКК](../consume-packages/migrate-packages-config-to-package-reference.md#create-a-package-after-migration).
...
Разделенный запятыми список авторов пакетов, использующих имена профилей в nuget.org. Часто это тот же список, что и в `authors`, и игнорируется при отправке пакета в nuget.org. См. раздел [Управление владельцами пакетов в NuGet.org](../nuget-org/publish-a-package.md#managing-package-owners-on-nugetorg). 
...
При использовании пользовательской лицензии, которая не поддерживается выражениями лицензий, можно упаковать файл `.txt` или `.md` с текстом лицензии. Пример:
...
> [!Tip]
> Можно указать и `icon`, и `iconUrl` для обеспечения обратной совместимости с источниками, которые не поддерживают `icon`. Visual Studio будет поддерживать `icon` пакетов, поступающих из источника на основе папок в будущем выпуске.
...
Метаданные репозитория, состоящие из четырех необязательных атрибутов: `type` и `url` *(4.0 +)* и `branch` и `commit` *(4.6 +)* . Эти атрибуты позволяют сопоставлять `.nupkg` с репозиторием, в котором он построен, с возможностью получения таких сведений, как имя отдельной ветви и/или фиксация хэша SHA-1, который создал пакет. Это должен быть общедоступный URL-адрес, который может вызываться непосредственно программным обеспечением управления версиями. Это не должна быть HTML-страница, так как она предназначена для компьютера. Для ссылки на страницу проекта используйте вместо этого поле `projectUrl`.
...
#### <a name="packagetypes"></a>Элемента packagetypes
...
Узел `<package>` может содержать узел `<files>` в качестве одноуровневого элемента для `<metadata>` и дочерний элемент `<contentFiles>` в `<metadata>` для указания файлов сборки и содержимого, включаемых в пакет. Дополнительные сведения см. далее в разделах [Включение файлов сборки](#including-assembly-files) и [Включение файлов содержимого](#including-content-files) этой статьи.
...
При создании пакета [команда `nuget pack`](../reference/cli-reference/cli-ref-pack.md) заменяет маркеры с разделителями $ в узле `<metadata>` файла `.nuspec` значениями из файла проекта или параметра `-properties` команды `pack`.
...
| Токен | Источник значения | значения
| --- | --- | ---
...
| `version` | Диапазон версий, которые допустимы в качестве зависимости (обязательно). Точный синтаксис см. в разделе [Управление версиями пакета](../concepts/package-versioning.md#version-ranges-and-wildcards). Версии с подстановочными знаками (плавающие) не поддерживаются. |
...
> [!Important]
> При создании `.nuspec` из проекта с помощью `nuget spec` зависимости, которые существуют в этом проекте, не включаются автоматически в результирующий файл `.nuspec`. Вместо этого используйте `nuget pack myproject.csproj` и получите файл *. nuspec* в созданном файле *nupkg* . Этот *nuspec* содержит зависимости.
...
| Атрибут | Описание |
| --- | --- |
| **include** | Расположение файла или файлов, которые требуется включить, с учетом исключений, задаваемых атрибутом `exclude` (обязательно). Путь задается относительно папки `contentFiles`, если не указан абсолютный путь. Допускается использовать подстановочный знак `*`. Наличие сдвоенного подстановочного знака `**` подразумевает выполнение рекурсивного поиска в папке. |
| **exclude** | Разделенный точками с запятой список файлов или шаблонов файлов, которые исключаются из расположения `src`. Допускается использовать подстановочный знак `*`. Наличие сдвоенного подстановочного знака `**` подразумевает выполнение рекурсивного поиска в папке. |
