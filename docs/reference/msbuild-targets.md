...
| Атрибут или nuspec значение | СвойствоMSBuild . | Значение по умолчанию | Примечания |
|--------|--------|--------|--------|
...
| `PackageType` | `<PackageType>CustomType1, 1.0.0.0;CustomType2</PackageType>` | | Указывает предполагаемое использование пакета. Типы пакетов используют тот же формат, что и идентификаторы пакетов, и разделяются `;` . Для типов пакетов можно указать версию, добавив `,` и [`Version`](/dotnet/api/system.version) строку. См. раздел [Установка NuGet типа пакета](../create-packages/set-package-type.md) ( NuGet 3.5.0 +). |
...
*Поддерживается с **NuGet 5.10.0 Preview 2**  /  **.NET SDK 5.0.300** и выше*
