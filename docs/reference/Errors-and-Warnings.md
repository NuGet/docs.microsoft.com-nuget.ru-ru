...
Перечисленные здесь ошибки и предупреждения доступны только в проектах [на основе PackageReference](../consume-packages/package-references-in-project-files.md) и NuGet 4.3.0 +. NuGet также учитывает свойства MSBuild, чтобы подавить предупреждения или повысить их до ошибок. Дополнительные сведения см. в разделе [инструкции. Отключение предупреждений компилятора](/visualstudio/ide/how-to-suppress-compiler-warnings) в документации по Visual Studio.
