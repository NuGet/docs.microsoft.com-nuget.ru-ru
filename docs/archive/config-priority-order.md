### <a name="priority-ordering"></a>Упорядочение по приоритетам

Кроме того, рекомендуется подумать о порядке применения параметров, который по своей сути является обратным порядку обработки. Например, если проект находится в `c:\A\B\C`, то NuGet применяет параметры в следующем порядке, где параметры имеют тем больший приоритет, чем выше они в списке:
