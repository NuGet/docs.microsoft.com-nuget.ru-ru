...
---
# <a name="package-versioning"></a>Пакет управления версиями
...
- `-beta`: Бета-выпуска, как правило, все для следующей функции запланированный выпуск, но может содержать известные ошибки.
- `-rc`: Версия-кандидат, как правило, потенциально окончательного выпуска (stable), если не выявляются существенных ошибок.

> [!Note]
> Поддерживает NuGet 4.3.0+ [SemVer 2.0.0](http://semver.org/spec/v2.0.0.html), поддерживающей предварительного выпуска чисел с помощью точечной нотации, как в *1.0.1-build.23*. Нотация с точками не поддерживается с версии NuGet до 4.3.0. Можно использовать в форме, как *1.0.1-build23*.
...
|----------|--------------|-------------|
| 1,0 | 1.0 ≤ x | Минимальная версия включительно |
| (1.0,) | 1.0 < x | Минимальная версия монопольного |
...
При использовании PackageReference или `project.json` пакета форматы ссылку, NuGet также поддерживает использование нотацию подстановочных знаков, \*основной, незначительное, исправление и частей предварительной версии суффикс числа. Подстановочные знаки не поддерживаются с `packages.config` формат.
