...
* **Возможные последствия**. Преобразования XDT (чаще всего `web.config.install.xdt` и `web.config.uninstall.xdt`) не применяются ни к каким XML-файлам проекта. Это означает, что файл ` web.config` проекта не обновляется при установке или удалении пакета.
