...
Установка пакета в консоли выполняет те же действия, как описано в статье [что происходит при установке пакета](../consume-packages/ways-to-install-a-package.md#what-happens-when-a-package-is-installed), с учетом следующих отличий:

- В консоли отображаются применимые условия лицензионных соглашений в своем окне неявные соглашения. Если вы не согласны с условиями, следует удалить пакет немедленно.
- Также ссылку на пакет добавляется в файл проекта и отображается в **обозревателе решений** под **ссылки** узла, необходимо сохранить проект, чтобы увидеть изменения в файл проекта напрямую.
...
- Удаляет ссылки на пакет из проекта (и формату управления он используется). Ссылки больше не отображались в **обозревателе решений**. (Может потребоваться перестроить проект, чтобы увидеть его удалить **Bin** папки.)
