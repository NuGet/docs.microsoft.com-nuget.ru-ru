...
| SymbolPackageFormat | Задает формат пакет символов: *symbols.nupkg* (устаревший) или *snupkg* (рекомендуется). По умолчанию она создает [пакета устаревших символов](../create-packages/Symbol-Packages.md). См. раздел [Создание пакетов символов (.snupkg)](../create-packages/Symbol-Packages-snupkg.md). |
