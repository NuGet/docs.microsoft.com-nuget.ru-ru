...
-------- | ------ | ------- | -------- | -----
LOWER_ID | URL-адрес    | строка  | да      | Идентификатор пакета, букв
...
Ниже  | строка           | да      | Самая ранняя версия SemVer 2.0.0 на странице (включительно)
parent | строка           | нет       | URL-адрес регистрации индекса
верхний  | строка           | да      | Самую новую версию SemVer 2.0.0 на странице (включительно)
...
Имя           | Тип   | Обязательно | Примечания
-------------- | ------ | -------- | -----
@id            | строка | да      | URL-адрес конечного регистрации
catalogEntry   | object | да      | Запись каталога, содержащего метаданные пакета
packageContent | строка | да      | URL-адрес содержимого пакета (.nupkg)
...
Имя                     | Тип                       | Обязательно | Примечания
------------------------ | -------------------------- | -------- | -----
@id                      | строка                     | да      | URL-адрес для документа, использованного для создания этого объекта
...
Опубликован                | строка                     | нет       | Строка, содержащая отметку времени ISO 8601, когда пакет опубликован
requireLicenseAcceptance | boolean                    | нет       | 
сводка                  | строка                     | нет       | 
теги                     | Строка или массив строк  | нет       | 
заголовок                    | строка                     | нет       | 
version                  | строка                     | да      | Версия пакета
...
Имя            | Тип             | Обязательно | Примечания
--------------- | ---------------- | -------- | -----
targetFramework | строка           | нет       | Требуемая версия .NET framework, применимые для этих зависимостей
...
Имя         | Тип   | Обязательно | Примечания
------------ | ------ | -------- | -----
идентификатор           | строка | да      | Идентификатор пакета зависимостей
range        | object | нет       | Допустимое [диапазон версий](../reference/package-versioning.md#version-ranges-and-wildcards) зависимости
регистрация | строка | нет       | URL-адрес регистрации индекс для этой зависимости
...
Ниже  | строка           | да      | Самая ранняя версия SemVer 2.0.0 на странице (включительно)
parent | строка           | да      | URL-адрес регистрации индекса
верхний  | строка           | да      | Самую новую версию SemVer 2.0.0 на странице (включительно)
...
-------------- | ------- | -------- | -----
@id            | строка  | да      | URL-адрес конечного регистрации
catalogEntry   | строка  | нет       | URL-адрес для записи каталога, которое вызвало эти конечные
перечисленные         | boolean | нет       | Следует рассматривать как перечисленных случае его отсутствия
packageContent | строка  | нет       | URL-адрес содержимого пакета (.nupkg)
Опубликован      | строка  | нет       | Строка, содержащая отметку времени ISO 8601, когда пакет опубликован
регистрация   | строка  | нет       | URL-адрес регистрации индекса
