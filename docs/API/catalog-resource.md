...
Индекс в каталоге является документом в хорошо известных расположении, которое содержит список элементов каталога, упорядочены в хронологическом порядке. Это точка входа ресурса каталога.
...
--------------- | ---------------- | -------- | -----
commitId        | строка           | да      | Уникальный идентификатор, связанный с самой последней фиксации
commitTimeStamp | строка           | да      | Отметка времени последней фиксации
...
Имя            | Тип    | Обязательно | Примечания
--------------- | ------- | -------- | -----
@id             | строка  | да      | URL-адрес страницы каталога выборки
commitId        | строка  | да      | Уникальный идентификатор, связанный с самой последней фиксацией на этой странице
commitTimeStamp | строка  | да      | Отметка времени последней фиксации на этой странице
...
parent          | строка           | да      | URL-адрес каталога
...
Количество элементов каталога на странице определяется реализация сервера. Для nuget.org не превышает 550 элементы на каждой странице, несмотря на то, что фактическое количество можно меньшего размера для некоторых страниц, в зависимости от размера следующего пакета фиксации в момент времени.
...
Имя            | Тип    | Обязательно | Примечания
--------------- | ------- | -------- | -----
@id             | строка  | да      | URL-адрес для получения элемента каталога
@type           | строка  | да      | Тип элемента каталога
commitId        | строка  | да      | Идентификатор фиксации, связанный с этим элементом каталога
commitTimeStamp | строка  | да      | Метка времени фиксации данного элемента каталога
NuGet:ID        | строка  | да      | Идентификатор пакета, связанного с этой конечного
NuGet:Version   | строка  | да      | Версия пакета, связанного с этой конечного
...
Конечный каталог содержит метаданные о конкретных ИД и версию пакета в определенный момент времени. Это документ, выбранных с помощью `@id` , найденного на странице каталога. URL-адрес конечного каталога не является прогнозируемым и должен быть обнаружен в каталоге страницы.
...
каталог: commitId        | строка                     | да      | Идентификатор фиксации, связанный с этим элементом каталога
catalog:commitTimeStamp | строка                     | да      | Метка времени фиксации данного элемента каталога
идентификатор                      | строка                     | да      | Идентификатор пакета элемента каталога
Опубликован               | строка                     | да      | Дата публикации элемента каталога пакета
version                 | строка                     | да      | Версия пакета элемента каталога
...
----------------------- | -------------------------- | -------- | -----
authors                 | строка                     | нет       |
created                 | строка                     | да      | Отметку времени, при первом создании пакета
...
packageHash             | строка                     | да      | Хэш-код пакета, кодирование с помощью [стандартные base-64](https://tools.ietf.org/html/rfc4648#section-4)
packageHashAlgorithm    | строка                     | да      |
...
verbatimVersion         | строка                     | нет       | Строка версии, как он изначально находится в .nuspec
...
`packageHashAlgorithm` — Это строка, определяются реализацией сервера, представляющий алгоритм хеширования, используемый для создания `packageHash`. всегда используется NuGet.org `packageHashAlgorithm` значение `SHA512`.

`published` Отметка времени — времени, когда последнего перечисленные пакета.

> [!Note]
> На nuget.org `published` имеет значение 1900 года, если пакет не включенных в список.
