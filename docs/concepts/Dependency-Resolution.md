...
Гибкая версия зависимости указывается с помощью знака \*. Например, `6.0.*`. Такая запись версии указывает, что нужно использовать самую последнюю версию 6.0.x. Если указано `4.*`, нужно использовать последнюю версию 4.x. Использование версий с подстановочным знаком сокращает число изменений в файле проекта, обеспечивая применение последней версии зависимости.
...
> [!Note]
> Сведения о работе гибких версий см. в разделе [Управление версиями пакетов](package-versioning.md#version-ranges).
...
При использовании формата PackageReference вы можете управлять тем, какие ресурсы из зависимостей переходят в проект верхнего уровня. Дополнительные сведения см. в разделе [Управление ресурсами зависимости](../consume-packages/package-references-in-project-files.md#controlling-dependency-assets).
