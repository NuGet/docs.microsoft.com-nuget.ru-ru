...
Имя           | Тип                       | Обязательно | Примечания
-------------- | -------------------------- | -------- | -----
идентификатор             | строка                     | yes      | Идентификатор сопоставленного пакета
version        | строка                     | yes      | Полная строка версии SemVer 2.0.0 пакета (может содержать метаданные сборки)
description    | строка                     | нет       | 
versions       | массив объектов           | yes      | Все версии пакета, соответствующие `prerelease` параметру
...
packageTypes   | массив объектов           | yes      | Типы пакетов, определяемые автором пакета (добавлены в `SearchQueryService/3.5.0` )
...
Имя      | Тип    | Обязательно | Примечания
--------- | ------- | -------- | -----
@id       | строка  | yes      | Абсолютный URL-адрес связанного [конечного листа регистрации](registration-base-url-resource.md#registration-leaf)
version   | строка  | yes      | Полная строка версии SemVer 2.0.0 пакета (может содержать метаданные сборки)
загрузки | Целое число | yes      | Число Скачиваний для конкретной версии пакета
