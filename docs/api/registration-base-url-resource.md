...
Об устаревании              | object                     | Нет       | Об устаревании, связанные с пакетом
Описание              | string                     | Нет       | 
iconUrl                  | string                     | Нет       | 
...
#### <a name="package-deprecation"></a>Об устаревании пакета

Устаревание каждый пакет имеет следующие свойства:

name             | Тип             | Обязательно | Примечания
---------------- | ---------------- | -------- | -----
причины          | Массив строк | да      | Причины, почему был объявлен устаревшим пакета
сообщение          | string           | Нет       | Дополнительные сведения об этом об устаревании
alternatePackage | object           | Нет       | Зависимость пакета, который должен использоваться вместо

`reasons` Свойство должно содержать по крайней мере одну строку и должен содержать только строки из следующей таблицы:

Причина       | Описание             
------------ | -----------
Прежние версии       | Пакет больше не поддерживается.
CriticalBugs | Пакет имеет ошибки, которые делают его непригодным для использования
Другое        | Пакет устарел из-за причин, отсутствующим в списке

Если `reasons` свойство содержит строки, которые не из набора известных, они будут пропускаться. Строки не учитывается регистр, поэтому `legacy` должно быть обрабатывается так же, `Legacy`. Не накладывается упорядочения в массиве, поэтому строки могут быть размещены в любой произвольный порядок. Кроме того Если это свойство содержит только те строки, которые не являются из набора известных, его следует рассматривать так, как если бы он содержал только строка «Other».
