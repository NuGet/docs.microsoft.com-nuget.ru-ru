...
Name     | В     | Тип    | Обязательное | Примечания
-------- | ------ | ------- | -------- | -----
LOWER_ID | {2&gt; URL&lt;2}    | string  | да      | Идентификатор пакета, в нижнем регистре
...
parent | string           | no       | URL-адрес индекса регистрации
...
Deprecation              | Объект                     | no       | Устаревшее значение, связанное с пакетом
Описание              | string                     | no       | 
iconUrl                  | string                     | no       | 
id                       | string                     | да      | Идентификатор пакета
licenseUrl               | string                     | no       |
лиценсикспрессион        | string                     | no       | 
списки                   | boolean                    | no       | Следует рассматривать как перечисленное, если отсутствует
...
сводка                  | string                     | no       | 
теги                     | строка или массив строк  | no       | 
заголовок                    | string                     | no       | 
Версия                  | string                     | да      | Строка полной версии после нормализации
...
range        | Объект | no       | Допустимый [диапазон версий](../concepts/package-versioning.md#version-ranges-and-wildcards) зависимости
...
сообщение          | string           | no       | Дополнительные сведения об этой нерекомендуемости
алтернатепаккаже | Объект           | no       | Вместо этого следует использовать альтернативный пакет
...
range        | Объект | no       | Допустимый [диапазон версий](../concepts/package-versioning.md#version-ranges-and-wildcards)или `*`, если разрешена любая версия
регистрация | string | no       | URL-адрес индекса регистрации для этого альтернативного пакета
