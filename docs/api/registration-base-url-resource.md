...
----- | ---------------- | -------- | -----
count | integer          | да      | Число страниц регистрации в индексе
элементов | массив объектов | да      | Массив страниц регистрации
...
count  | integer          | да      | Число конечных элементов регистрации на странице
элементов  | массив объектов | no       | Массив конечных элементов регистрации и их связанные метаданные
lower  | string           | да      | Самая низкая версия SemVer 2.0.0 на странице (включающая)
родитель | string           | no       | URL-адрес индекса регистрации
upper  | string           | да      | Самая высокая версия SemVer 2.0.0 на странице (включающая)
...
депенденциграупс         | массив объектов           | no       | Зависимости пакета, сгруппированные по целевым платформам
Deprecation              | object                     | no       | Устаревшее значение, связанное с пакетом
description              | string                     | no       | 
iconUrl                  | string                     | no       | 
id                       | string                     | да      | Идентификатор пакета
licenseUrl               | string                     | no       |
лиценсикспрессион        | string                     | no       | 
списки                   | Логическое                    | no       | Следует рассматривать как перечисленное, если отсутствует
minClientVersion         | string                     | no       | 
projectUrl               | string                     | no       | 
published                | string                     | no       | Строка, содержащая метку времени ISO 8601, когда пакет был опубликован
requireLicenseAcceptance | Логическое                    | no       | 
summary                  | string                     | no       | 
...
Значение свойства `licenseExpression` соответствует [синтаксису выражения лицензии NuGet](../reference/nuspec.md#license).
...
Имя            | Тип             | Обязательно | Примечания
--------------- | ---------------- | -------- | -----
targetFramework | string           | no       | Целевая платформа, к которой применяются эти зависимости
зависимости    | массив объектов | no       |
...
range        | object | no       | Допустимый [диапазон версий](../concepts/package-versioning.md#version-ranges) зависимости
регистрация | string | no       | URL-адрес индекса регистрации для этой зависимости
...
message          | string           | no       | Дополнительные сведения об этой нерекомендуемости
алтернатепаккаже | object           | no       | Вместо этого следует использовать альтернативный пакет
...
Причина       | Описание             
------------ | -----------
Устаревший       | Пакет больше не поддерживается
критикалбугс | Пакет содержит ошибки, которые делают его непригодным для использования
Прочее        | Пакет устарел из-за причины, не включенной в этот список
...
range        | object | no       | Допустимый [диапазон версий](../concepts/package-versioning.md#version-ranges)или `*`, если разрешена любая версия
...
@id    | string           | да      | URL-адрес страницы регистрации
count  | integer          | да      | Число конечных элементов регистрации на странице
элементов  | массив объектов | да      | Массив конечных элементов регистрации и их связанные метаданные
lower  | string           | да      | Самая низкая версия SemVer 2.0.0 на странице (включающая)
родитель | string           | да      | URL-адрес индекса регистрации
upper  | string           | да      | Самая высокая версия SemVer 2.0.0 на странице (включающая)
...
-------------- | ------- | -------- | -----
@id            | string  | да      | URL-адрес конечного листа регистрации
каталожентри   | string  | no       | URL-адрес записи каталога, которая создала эти конечные
списки         | Логическое | no       | Следует рассматривать как перечисленное, если отсутствует
паккажеконтент | string  | no       | URL-адрес содержимого пакета (. nupkg)
published      | string  | no       | Строка, содержащая метку времени ISO 8601, когда пакет был опубликован
регистрация   | string  | no       | URL-адрес индекса регистрации
