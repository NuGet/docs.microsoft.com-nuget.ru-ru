...
-------- | ------ | ------- | -------- | -----
LOWER_ID | URL-адрес    | строка  | да      | Идентификатор пакета, в нижнем регистре

`LOWER_ID`Значение представляет собой требуемый идентификатор пакета в нижнем регистре, используя правила, реализованные. Метод NET [`System.String.ToLowerInvariant()`](/dotnet/api/system.string.tolowerinvariant?view=netstandard-2.0#System_String_ToLowerInvariant) .
...
----- | ---------------- | -------- | -----
count | Целое число          | да      | Число страниц регистрации в индексе
...
Имя   | Тип             | Обязательно | Примечания
------ | ---------------- | -------- | -----
@id    | строка           | да      | URL-адрес страницы регистрации
count  | Целое число          | да      | Число конечных элементов регистрации на странице
items  | массив объектов | Нет       | Массив конечных элементов регистрации и их связанные метаданные
lower  | строка           | да      | Самая низкая версия SemVer 2.0.0 на странице (включающая)
родитель | строка           | Нет       | URL-адрес индекса регистрации
upper  | строка           | да      | Самая высокая версия SemVer 2.0.0 на странице (включающая)
...
Имя           | Тип   | Обязательно | Примечания
-------------- | ------ | -------- | -----
@id            | строка | да      | URL-адрес конечного листа регистрации
каталожентри   | объект | да      | Запись каталога, содержащая метаданные пакета
паккажеконтент | строка | да      | URL-адрес содержимого пакета (. nupkg)
...
Имя                     | Тип                       | Обязательно | Примечания
------------------------ | -------------------------- | -------- | -----
@id                      | строка                     | да      | URL-адрес документа, используемого для создания этого объекта
...
iconUrl                  | строка                     | Нет       | 
идентификатор                       | строка                     | да      | Идентификатор пакета
...
title                    | строка                     | Нет       | 
version                  | строка                     | да      | Строка полной версии после нормализации
...
Имя         | Тип   | Обязательно | Примечания
------------ | ------ | -------- | -----
идентификатор           | строка | да      | Идентификатор зависимости пакета
range        | объект | Нет       | Допустимый [диапазон версий](../concepts/package-versioning.md#version-ranges) зависимости
...
Другие        | Пакет устарел из-за причины, не включенной в этот список
...
Имя         | Тип   | Обязательно | Примечания
------------ | ------ | -------- | -----
идентификатор           | строка | да      | ИДЕНТИФИКАТОР альтернативного пакета
range        | объект | Нет       | Допустимый [диапазон версий](../concepts/package-versioning.md#version-ranges)или, `*` Если разрешена любая версия
...
lower  | строка           | да      | Самая низкая версия SemVer 2.0.0 на странице (включающая)
родитель | строка           | да      | URL-адрес индекса регистрации
upper  | строка           | да      | Самая высокая версия SemVer 2.0.0 на странице (включающая)
