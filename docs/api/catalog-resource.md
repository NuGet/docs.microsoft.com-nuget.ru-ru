...
--------------- | ---------------- | -------- | -----
коммитид        | строка           | yes      | Уникальный идентификатор, связанный с последней фиксацией
коммиттиместамп | строка           | yes      | Метка времени последней фиксации
count           | Целое число          | yes      | Число страниц в индексе
...
Имя            | Тип    | Обязательно | Примечания
--------------- | ------- | -------- | -----
@id             | строка  | yes      | Страница "URL-адрес для выборки каталога"
коммитид        | строка  | yes      | Уникальный идентификатор, связанный с последней фиксацией на этой странице
коммиттиместамп | строка  | yes      | Метка времени последней фиксации на этой странице
count           | Целое число | yes      | Число элементов на странице каталога
...
коммитид        | строка           | yes      | Уникальный идентификатор, связанный с последней фиксацией на этой странице
коммиттиместамп | строка           | yes      | Метка времени последней фиксации на этой странице
count           | Целое число          | yes      | Число элементов на странице
items           | массив объектов | yes      | Элементы каталога на этой странице
родитель          | строка           | yes      | URL-адрес индекса каталога
...
Имя            | Тип    | Обязательно | Примечания
--------------- | ------- | -------- | -----
@id             | строка  | yes      | URL-адрес для выборки элемента каталога
@type           | строка  | yes      | Тип элемента каталога
коммитид        | строка  | yes      | Идентификатор фиксации, связанный с этим элементом каталога
коммиттиместамп | строка  | yes      | Метка времени фиксации этого элемента каталога
NuGet: ИД        | строка  | yes      | Идентификатор пакета, с которым связан этот конечный объект
NuGet: версия   | строка  | yes      | Версия пакета, с которой связан этот конечный объект
...
Каталог: Коммитид        | строка                     | yes      | Идентификатор фиксации, связанный с этим элементом каталога
Каталог: Коммиттиместамп | строка                     | yes      | Метка времени фиксации этого элемента каталога
идентификатор                      | строка                     | yes      | Идентификатор пакета для элемента каталога
published               | строка                     | yes      | Дата публикации элемента каталога пакетов
version                 | строка                     | yes      | Версия пакета для элемента каталога
...
паккажехаш             | строка                     | yes      | Хэш пакета, кодировка с использованием [стандартного базового 64](https://tools.ietf.org/html/rfc4648#section-4)
паккажехашалгорисм    | строка                     | yes      |
паккажесизе             | Целое число                    | yes      | Размер пакета. nupkg в байтах
