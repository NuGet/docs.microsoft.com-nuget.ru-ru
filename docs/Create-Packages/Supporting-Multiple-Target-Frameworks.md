...
*Сведения о межплатформенном нацеливании для проектов .NET Core, использующих NuGet 4.0 или более поздней версии, см. в разделе [Создание и восстановление пакетов NuGet в качестве целей MSBuild](../schema/msbuild-targets.md).*
...
- [Структура папок, соответствующих версиям платформы](#framework-version-folder-structure)
- [Сопоставление версий сборки и целевых платформ в проекте](#matching-assembly-versions-and-the-target-framework-in-a-project)
- [Группирование сборок по версии платформы](#grouping-assemblies-by-framework-version)
- [Определение требуемой цели NuGet](#determining-which-nuget-target-to-use)
- [Файлы содержимого и скрипты PowerShell](#content-files-and-powershell-scripts)
...
Полный список поддерживаемых имен см. в [справочнике по целевым платформам](../schema/target-frameworks.md#supported-frameworks).
...
> [!Warning]
> Изменяемые файлы содержимого и выполнение скриптов можно использовать только с форматом `packages.config`. Они являются нерекомендуемыми при использовании форматов `project.json` и PackagesReference, и их не следует применять для новых пакетов.
