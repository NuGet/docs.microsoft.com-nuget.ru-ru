...
Когда NuGet устанавливает пакет с файлами `\build`, он добавляет в файл проекта элементы MSBuild `<Import>`, указывающие на файлы `.targets` и `.props`. (Файлы `.props` добавляются в начале файла проекта, а файлы `.targets` — в конце.)
